...
Status: Accepted; extended by ADR-0009 · Date: 2026-08-17 · Supersedes: —

## Context

Stage gates depend on how far reasoning has completed, while semantic IR deliberately
...
declared IR roles to relative paths and their canonical schema `$id`. The Model Repository
...
- Artifact names and directory layout may evolve without changing canonical IR references.
  ...
  − The manifest adds a non-semantic schema that must itself retain compatibility and field
  traces.
